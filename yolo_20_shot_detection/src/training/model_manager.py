...
class ModelManager:
    """
    Model lifecycle utilities for Ultralytics YOLO.

    Responsibilities
    ---------------
    - Load a pre-trained checkpoint or model alias
    - Resolve device preferences from configuration
    - Optionally freeze early modules for transfer learning
    - Provide basic model parameter summaries
    """
    ...
    def _resolve_device(self, device_str: str) -> torch.device:
        ...
        Supports common patterns like "cpu", "cuda:0", "0", and "0,1"
        (the first ID is used to pick the primary device for model init).
        """
    ...
    def create_model(self) -> YOLO:
        """
        Load the specified YOLO model and freeze early modules if requested.

        Note: Ultralytics adapts the detection head automatically during
        training/validation based on the data yaml (number of classes).
        """
        try:
    ...
    def _freeze_layers(self, num_layers_to_freeze: int):
        """
        Freeze gradients for the first `num_layers_to_freeze` modules.

        Implementation targets the list-like container at
        `self.model.model.model`, which typically holds backbone blocks.
        """
    ...
    def log_model_info(self):
        """
        Print a summary of total, trainable, and frozen parameter counts.
        """
