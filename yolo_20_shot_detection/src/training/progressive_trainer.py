...
class ProgressiveCallbackTrainer(DetectionTrainer):
    """
    Custom Ultralytics trainer that injects progressive-training behaviors via callbacks.

    Adds three capabilities on top of the base trainer:
    - On-the-fly RandAugment for novel images
    - Interval-based validation
    - Early phase stopping using a mAP threshold
    """
    ...
    def on_train_batch_start(self, batch):
        """Applies RandAugment on-the-fly to novel-class images only."""
    ...
    def on_fit_epoch_end(self, epoch):
        """Triggers validation at a specified interval."""
    ...
    def on_val_end(self):
        """Checks if the model has reached a performance plateau to stop a phase early."""
...
class ProgressiveTrainer:
    """
    Orchestrate the multi-phase training and custom evaluation workflow.

    Coordinates dataset phase creation, augmentation setup, training, validation,
    and model selection using a safety-net rule on base-class performance.
    """
    ...
    def run_training(self):
        """
        Execute the full progressive training workflow, phase by phase.
        """
    ...
    def _prepare_phase_metadata_and_augs(self, phase_yaml_path: Path):
        """
        Load per-image novelty metadata and configure augmentations for the phase.
        """
    ...
    def _build_trainer_args(self, phase_yaml_path: Path, phase_num: int) -> Dict[str, Any]:
        """
        Construct the dictionary of arguments for the Ultralytics trainer.
        """
    ...
    def _process_phase_results(self, trainer, phase_num):
        """
        Process results after each phase using the safety-net selection rule.
        """
    ...
    def _finalize_training(self):
        """
        Save the final best model, merged training history, and summary plots.
        """
