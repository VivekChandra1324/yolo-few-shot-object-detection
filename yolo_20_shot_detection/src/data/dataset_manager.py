...
class DatasetManager:
    """
    High-level orchestration of the data pipeline.

    Responsibilities
    ---------------
    - Download source datasets from Hugging Face (if missing)
    - Parse `dataset.yaml` files to obtain class names
    - Combine base and novel datasets into a single YOLO-formatted directory
    - Create per-phase train lists for progressive training
    - Provide paths and metadata required by training and evaluation
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        ----------
        config : Dict[str, Any]
            Global configuration (merged base + stage). Keys used include
            output_dir, dataset.hf_repos, seed, etc.
        """
        self.config = config
    ...
    def setup_datasets(self, task_type: str) -> None:
        """
        Prepare the combined dataset for a given task type.

        If an existing processed dataset is found, it is loaded; otherwise the
        source datasets are downloaded (if needed), parsed, and combined.
        """
        ...
        novel_repo_key = "homeobjects_143" if task_type == "task1" else "homeobjects_29"
    ...
    def _load_existing_dataset(self, task_type: str) -> None:
        """
        Load configuration from an existing, already-processed dataset.
        """
        master_yaml_path = self.combined_data_root / "dataset.yaml"
    ...
    def _prepare_source_dataset(self, repo_key: str, target_dir: Path) -> Tuple[List[str], Path]:
        """
        Download, extract, and parse a source dataset.

        Returns
        -------
        Tuple[List[str], Path]
            (class_names, path_to_dataset_yaml)
        """
    ...
    def _download_and_extract(self, repo_id: str, filename: str, target_dir: Path):
        """
        Download and extract a dataset from Hugging Face Hub if not present.
        """
    ...
    def _parse_dataset_yaml(self, search_path: Path) -> Tuple[List[str], Path]:
        """
        Find and parse the `dataset.yaml` file within an extracted directory.
        """
        try:
    ...
    def _create_master_yaml(self):
        """
        Create the main `dataset.yaml` file for the combined dataset.
        """
    ...
    def create_phase_training_yaml(self, novel_ratio: float) -> Path:
        """
        Create a phase-specific data YAML and metadata for progressive training.
        """
