...
class Evaluator:
    """
    Final model evaluation on the test set with base vs. novel breakdown.

    - Executes Ultralytics validation using the combined dataset `dataset.yaml`.
    - Splits mAP results between base and novel class groups for reporting.
    - Produces both decimal and percentage summaries for readability.
    """
    ...
    def evaluate_model(self, model: YOLO) -> Dict[str, Any]:
        """Run validation on the test split and return parsed summary metrics."""
    ...
    def _parse_and_log_metrics(self, metrics: DetMetrics) -> Dict[str, Any]:
        """Parse DetMetrics into overall/base/novel aggregates and log summaries."""
