...
Provides helpers to:
- Load and deep-merge base/stage YAMLs.
- Validate presence and types of critical keys.
- Summarize effective configuration for quick inspection.
"""
...
def merge_configs(base_config: Dict[str, Any], task_config: Dict[str, Any]) -> Dict[str, Any]:
    """Deep-merge a stage config into the base configuration dict."""
...
def load_and_merge_configs(base_config_path: str, task_config_path: str) -> Dict[str, Any]:
    """Load YAMLs from disk and return their deep-merged configuration dict."""
...
def validate_base_config(config: Dict[str, Any]) -> List[str]:
    """Validate core keys and types; return a list of error strings (if any)."""
    errors = []
...
def _is_valid_gpu_string(device_str: str) -> bool:
    """Return True if string encodes valid GPU identifiers (e.g., '0,1' or 'cuda:0')."""
    try:
        if ',' in device_str:
...
def _validate_dataset_config(dataset_config: Dict[str, Any]) -> List[str]:
    """Validate dataset section (expects HF repos metadata to be present)."""
    errors = []
...
def _validate_randaugment_config(randaugment_config: Dict[str, Any]) -> List[str]:
    """Validate RandAugment sub-config when enabled; check required keys and types."""
    errors = []
...
def validate_config_file(config_path: Path) -> bool:
    """Load a config file and validate it, returning True/False accordingly."""
    try:
        import yaml
...
def load_and_validate_config(config_path: str) -> Dict[str, Any]:
    """Load a config file and raise on validation failures; return the dict on success."""
    import yaml
...
def calculate_scaled_learning_rate(base_lr: float, base_batch_size: int, new_batch_size: int) -> float:
    """Apply linear scaling rule to adjust LR proportionally to batch size."""
...
def print_config_summary(config: Dict[str, Any]) -> None:
    """Log a concise summary of salient configuration values for verification."""
