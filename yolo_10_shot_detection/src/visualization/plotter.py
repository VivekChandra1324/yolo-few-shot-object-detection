...
def view_sample_images(dataset, num_samples: int = 4, class_names: dict = None):
    """
    Display a grid of random samples with ground truth bounding boxes.

    Useful for quick dataset inspection before training to verify
    image loading and label correctness.

    Args:
        dataset: PyTorch Dataset (e.g., CombinedYOLODataset).
        num_samples: Number of images to display in the grid.
        class_names: Optional mapping from class IDs to display names.
    """
    if not class_names:
    ...
    for i, ax_idx in enumerate(range(len(indices))):
        ...
        for box in labels:
            ...
            # Improved text positioning to prevent labels from going off-screen
...
def save_training_plots(history_df: pd.DataFrame, output_dir: Path, show: bool = True, task_name: str = None):
    """
    Generate and save a 2x2 training summary plot from history CSV.

    Creates plots for: box loss (train/val), mAP@0.5, mAP@0.5:0.95, and LR schedule.
    Handles missing columns gracefully and marks phase transitions when present.

    Args:
        history_df: Training history DataFrame (from Ultralytics results.csv).
        output_dir: Directory to save the plot image.
        show: Whether to display the plot interactively.
        task_name: Optional task name for unique output filename.
    """
    if history_df.empty:
    ...
    try:
        ...
        # Simplified and more robust pattern for finding the learning rate column
...
def visualize_inference(model, image_paths: list, num_samples: int = 4, conf_thresh: float = 0.25):
    """
    Run inference on sample images and display results with bounding boxes.

    Args:
        model: Trained Ultralytics YOLO model.
        image_paths: List of image file paths for inference.
